...
async function retrieveChunks(queries: string[]): Promise<SearchHit[]> {
  ...
  if (deduped.length === 0) {
    throw new Error('Vector search returned no results. Check OPENAI_API_KEY and NEXT_PUBLIC_SUPABASE_URL/NEXT_PUBLIC_SUPABASE_PROJECT_URL are set, and knowledge_chunks has rows.')
  }
